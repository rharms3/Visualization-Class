...
var x = d3.scaleLog().domain([0,500]).range([0,500]);
var y = d3.scaleLog().domain([0,500]).range([500,0]);
const margin = ( 50 );
const height = ( 500 );
const width = ( 500 );
const radplus = ( 2 );
...
//crime_data.then(function(data) {
  d3.select("svg")
    ...
    .filter(function(data) { return data.year == 2015})
	  .append('circle')
	    .attr('cx',function(data) {return xs(data.total_crime);}) //xs
	    .attr('cy',function(data) {return ys(data.population);})  //ys
	    .attr('r',function(data) {return +radplus + +5;})
